import 'package:flutter/material.dart';
import 'package:flutter_app/classes/application/application_state.dart';
import 'package:flutter_app/widgets/buttons/main_buttons/main_button_styles/button_style.dart';

class NormalButtonStyle extends AbstractButtonStyle {
  @override
  Color get background => ApplicationState().theme.primary300Color;

  @override
  BorderRadius get borderRadius =>  ApplicationState().theme.borderRadius;

  @override
  Color get foreground => ApplicationState().theme.primary600Color;
  
  @override
  Color get accent => ApplicationState().theme.primary200Color;
}